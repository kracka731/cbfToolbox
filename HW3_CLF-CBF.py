...
def main():
    v_l = 1 # leading vehicle velocity 
    v_d = 1.5 # desired velocity 
    tau = 1
    ...
    g1 = Goal(np.array([v_d, tau*v_l]), dynamics=const_vel, gamma=1, H=np.array([[1, 0], [0, 1]]))
    g1.color = 'blue'

    # Create an obstacle --> CBF 
    leading_vehicle = Obstacle(state=np.array([v_l, tau*v_l]), shape=Point(), dynamics=const_vel, p=2)
